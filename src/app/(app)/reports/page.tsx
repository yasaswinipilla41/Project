 ...
 * Reports (§35).
 *
 * Every figure is a database aggregation over the caller's accessible
 * projects — grouped queries rather than a count per row, so the page cost does
 * not grow with the number of issues.
 */
/**
 * Reference timestamps for the report window.
 *
 * Kept outside the component so the render body stays free of clock reads —
 * the same reason React's rules flag `new Date()` during render.
 */
