 ...
 *
 *   light — a pale surface (the content well)
 *   dark  — the brand's own dark chrome (the sign-in splash), dark in every theme
 *   nav   — the navigation surface, which follows the reader's theme
 *
 * `nav` exists because the sidebar is no longer dark in light mode. Passing
 * `dark` there would paint a white wordmark on a white sidebar.
 */
export type PrioLogoTone = "light" | "dark" | "nav";
...
export interface PrioLogoProps {
  ...
  size?: PrioLogoSize;
  /** `dark` = the brand's dark chrome; `nav` = the themed navigation surface. */
  tone?: PrioLogoTone;
